...
const clearAndFetchEBSVolumes = () => {
    return new Promise(async (resolve, reject) => {
        console.log(`scanning DB for EBS volume records...`);
        try {
            let ebsDynamoRecords = await getDynamoEBSVolumes();
            console.log(`found ${ebsDynamoRecords.length} EBS records in the DB...`);

            // Clear old EC2 records from dynamo table
            if (ebsDynamoRecords.length > 0) {
                console.log(`deleting ${ebsDynamoRecords.length} records...`);
                await deleteRecordsByPK(
                    CONSTANTS.TABLE_NAME,
                    ebsDynamoRecords.map((ebr) => ebr[CONSTANTS.PRIMARY_KEY_NAME])
                );
                console.log(`deleted all EBS volume records from the DB`);
            }
            ...
            ec2.describeVolumes(params, (err, data) => {
                ...
                try {
                    if (Array.isArray(data.Volumes) && data.Volumes.length > 0) {
                        console.log(`fetched ${data.Volumes.length} EBS records...`);
                        ebsRecords = [...data.Volumes, ...ebsRecords];
                    }

                    if (data.NextToken) {
                        params.NextToken = data.NextToken;
                        console.log("getting another page of EBS volume records");
                        getNextPage();
                    } else {
                        console.log("All EBS volumes are fetched");
